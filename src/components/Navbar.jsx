...
const Navbar = () => {
  ...
  return (
    <header
      className={`relative text-black m-4 sm:m-10 rounded-full z-50 ${
        isMenuOpen ? "rounded-xl h-auto" : "h-24"
    ...
    >
      <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-10 flex h-full text-base font-normal">
        <div className="flex justify-between items-center w-full">
          ...
          <button
            className={`lg:hidden m-2 rounded-full group transition-all easy-in inline-flex w-12 h-12 text-slate-800 bg-white text-center items-center justify-center ${
            ...
            >
              ...
              <rect
                className={`origin-center translate-y-[5px] transition-all duration-300 ease-[cubic-bezier(.5,.85,.25,1.1)] ${
                  isMenuOpen ? "translate-y-0 -rotate-[225deg]" : ""
                }`}
                y="7"
                width="9"
                height="2"
            ...
            {menuItems.map((item) => (
              <div key={item.text} className="w-8/12 m-0">
                {!item.subitems ? (
                  <a
                    href={item.href}
                    className={`${buttonClasses + "inline-block"} w-full`}
                  >
                    {item.text}
                  </a>
                ) : (
                  <div className="relative">
                    ...
                    {openDropdown === item.text && (
                      <div className=" p-4 ring-2 ring-custom-blue space-y-3 bg-white shadow-lg rounded-[1.5rem] z-10">
                        {item.subitems.map((subitem) => (
                          <a
                            key={subitem.text}
                            href={subitem.href}
